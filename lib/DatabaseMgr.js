...
class DatabaseMgr {
  ...
  const query = {
    name: 'getAdminDates',
    text: 'SELECT * FROM adminSetDates INNER JOIN(SELECT max(prodadmin_setdates_id) AS max_id FROM adminSetDates) a ON adminSetDates.prodadmin_setdates_id = a.max_id',
    values: []
  }
  ...
  const query = {
    name: 'create-account',
    text: 'INSERT INTO ACCOUNT_CREATE (produser_id, project_category, project_problem, project_approach, project_stage, project_impact, funding_goal, team_info) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) Returning *',
  ...
  const query = {
    name: 'createComment',
    text: 'INSERT into project_comments (prodproject_id, produser_id, submission_date, comment) VALUES ($1, $2 ,$3, $4) RETURNING *',
  ...
  const query = {
    ...
    text: 'select pc.comment_id, pc.prodproject_id, pc.produser_id, pc.submission_date, pc.comment, u.user_type, u.name, u.email from project_comments pc inner join users u on pc.produser_id = u.produser_id::text where pc.prodproject_id = $1',
  ...
  const query = {
    name: 'get-project',
    text: 'SELECT * FROM PROJECTS_DET WHERE prodproject_id = $1 and (record_status IS NULL OR record_status != \'not_confirmed\')',
    values: [projectId]
  }
  ...
  const query = {
    name: 'get-project',
    text: 'SELECT * FROM USERS WHERE produser_id = $1',
    values: [body.userId]
  }
...
//
 ...
 const client = new Client({
    ...
    //prodproject_id is autoincremented 
 ...
 const client = new Client({
  ...
  async updateUserCreds(body) {
   ...
   const query = {
    text: 'UPDATE USERS SET password = $1 WHERE produser_id = $2 RETURNING *',
  ...
  async promoteProject(body) {
   ...
   const query = {
    text: 'UPDATE projects_det SET record_status = \'pending_event\', status = \'PROMOTED\', listinghash = $2, promoter_publickey = $3, promoter_userid = $4 WHERE prodproject_id = $1 RETURNING *',
  ...
  const client = new Client({
   ...
   const query = {
    name: 'promote-project',
    text: 'UPDATE projects_det SET record_status = \'\', status = \'APPLIED\', listinghash = \'\', promoter_publickey = \'\' WHERE prodproject_id = $1 RETURNING *',
    values: [body.projectId]
  }
  ...
  const query = {
    name: 'getChallenge',
    text: 'SELECT * from CHALLENGES where prodproject_id = $1',
    values: [body.projectId]
  }
  ...
  const query = {
    name: 'challenge',
    text: 'INSERT INTO CHALLENGES (prodproject_id,  submission_date, listinghash, challenger, record_status) VALUES($1,$2,$3,$4,$5) RETURNING *',
  ...
  const query = {
    ...
    text: 'DELETE FROM CHALLENGES where (prodproject_id = $1 and listingHash = $2 and submission_date = $3) RETURNING *',
  ...
  const query = {
    name: 'challenge',
    text: 'UPDATE PROJECTS_DET set status = \'IN CHALLENGE PERIOD\', record_status = \'confirmed\' where prodproject_id = $1 RETURNING *',
    values: [body.projectId]
  }
 ...
 // body.status='ACCEPTED',
  ...
  const query = {
    name: 'getVotes',
    text: 'select max(prodvote_id) as prodvote_id, poll_id, secret_hash, salt, prodproject_id, produser_id, voter_publickey, record_status, status, voteoption, commitdate, revealdate from votes where prodproject_id = $1 and poll_id = $2 group by poll_id, secret_hash, salt, prodproject_id, produser_id, voter_publickey, record_status, status, voteoption, commitdate, revealdate',
  ...
  const query = {
    name: 'getVotesUser',
    text: 'select max(prodvote_id) as prodvote_id, poll_id, secret_hash, salt, prodproject_id, produser_id, voter_publickey, record_status, status, voteoption, commitdate, revealdate from votes where prodproject_id = $1 and poll_id = $2 and produser_id = $3 group by poll_id, secret_hash, salt, prodproject_id, produser_id, voter_publickey, record_status, status, voteoption, commitdate, revealdate',
  ...
  const query = {
    name: 'commitVote',
    text: 'INSERT INTO VOTES (poll_id, secret_hash, salt, voteoption, prodproject_id, produser_id, voter_publickey, record_status, status) VALUES($1,$2,$3,$4, $5, $6, $7, $8, $9) RETURNING *',
  ...
  const query = {
    ...
    text: 'DELETE from VOTES where (poll_id = $1 and prodproject_id = $2 and produser_id = $3) and prodvote_id = (select max(prodvote_id) from votes where poll_id = $1 and produser_id = $3 and prodproject_id = $2) RETURNING *',
  ...
  const client = new Client({
   ...
   const query = {
    ...
    text: 'UPDATE VOTES SET record_status = \'confirmed\' WHERE (poll_id = $1 and voter_publickey = $2) and prodvote_id = (select max(prodvote_id) from votes where poll_id = $1 and voter_publickey = $2) RETURNING *',
  ...
  const client = new Client({
   ...
   const query = {
    name: 'voteCommitEventConfirmed',
    text: 'UPDATE VOTES SET record_status = \'pending_event\', status = \'REVEALED\' WHERE (poll_id = $2 and produser_id = $1) RETURNING *',
  ...
  const client = new Client({
   ...
   const query = {
    ...
    text: 'UPDATE VOTES SET record_status = \'pending_event\', status = \'COMMITTED\' WHERE (poll_id = $2 and voter_publickey = $1) and prodvote_id = (select max(prodvote_id) from votes where poll_id = $2 and voter_publickey = $1) RETURNING *',
    values: [body.voterPublicKey, body.pollId]
  }
