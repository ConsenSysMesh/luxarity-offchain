...
//
 ...
 const client = new Client({
   ...
   const query = {
    ...
    values: [body.name, body.email, body.password, 'curator']
  }
  ...
  const query = {
    name: 'challenge',
    text: 'UPDATE projects_det set status = \'IN COMMIT VOTE PERIOD\', record_status = \'pending_event\' where listinghash = $1 RETURNING *',
    values: [body.listingHash]
  }
  ...
  const query = {
    ...
    text: 'UPDATE  projects_det set record_status = $2, maxchallenge_id = $3, max_commitenddate = $4, max_revealenddate = $5 WHERE listinghash = $1 RETURNING *',
    values: [body.listingHash, 'confirmed', body.challengeId, body.commitEndDate, body.revealEndDate]
  }
 ...
 // body.status='ACCEPTED',
  ...
  const query = {
    ...
    text: 'UPDATE projects_det SET userscommitedvotes = array_remove(userscommitedvotes, $1) where project_id = $2 RETURNING *',
    values: [body.userId, body.projectId]
  }
  ...
  const query = {
    ...
    text: 'UPDATE projects_det SET usersCommitedvotes = array_append(usersCommitedvotes, $1) WHERE project_id = $2 RETURNING *',
    values: [body.userId, body.projectId]
  }
