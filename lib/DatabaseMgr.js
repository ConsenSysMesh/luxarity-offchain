...
//
 ...
 // body.status='ACCEPTED',
  ...
  const query = {
    ...
    values: [body.pollId, body.secretHash, body.salt, body.projectId,  body.userId,  body.voterPublicKey, 'pending_event', 'COMMITTED']
  }
  ...
  const client = new Client({
   ...
   const query = {
    name: 'confirmRevealVote',
    text: 'UPDATE VOTES SET record_status = \'pending_event\', status = \'COMMITTED\' WHERE (poll_id = $2 and voter_publickey = $1) RETURNING *',
    values: [body.voterPublicKey, body.pollId]
  }
