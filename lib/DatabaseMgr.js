...
class DatabaseMgr {
  ...
  const query = {
    name: 'createComment',
    text: 'INSERT into project_comments (product_id, user_id, submission_date, comment) VALUES ($1, $2 ,$3, $4) RETURNING *',
  ...
  const query = {
    ...
    text: 'select pc.comment_id, pc.product_id, pc.user_id, pc.submission_date, pc.comment, u.user_type, u.name, u.email from project_comments pc inner join users u on pc.user_id = u.user_id::text where pc.product_id = $1',
  ...
  const query = {
    name: 'get-project',
    text: 'SELECT * FROM PROJECTS_DET WHERE product_id = $1 and (record_status IS NULL OR record_status != \'not_confirmed\')',
    values: [projectId]
  }
...
//
 ...
 const client = new Client({
  ...
  async promoteProject(body) {
   ...
   const query = {
    text: 'UPDATE projects_det SET record_status = \'pending_event\', status = \'PROMOTED\', listinghash = $2, promoter_publickey = $3, promoter_userid = $4 WHERE product_id = $1 RETURNING *',
  ...
  const client = new Client({
   ...
   const query = {
    name: 'promote-project',
    text: 'UPDATE projects_det SET record_status = \'\', status = \'APPLIED\', listinghash = \'\', promoter_publickey = \'\' WHERE product_id = $1 RETURNING *',
    values: [body.projectId]
  }
  ...
  const query = {
    name: 'getChallenge',
    text: 'SELECT * from CHALLENGES where product_id = $1',
    values: [body.projectId]
  }
  ...
  const query = {
    name: 'challenge',
    text: 'INSERT INTO CHALLENGES (product_id,  submission_date, listinghash, challenger, record_status) VALUES($1,$2,$3,$4,$5) RETURNING *',
  ...
  const query = {
    ...
    text: 'DELETE FROM CHALLENGES where (product_id = $1 and listingHash = $2 and submission_date = $3) RETURNING *',
  ...
  const query = {
    name: 'challenge',
    text: 'UPDATE PROJECTS_DET set status = \'IN CHALLENGE PERIOD\', record_status = \'confirmed\' where product_id = $1 RETURNING *',
    values: [body.projectId]
  }
 ...
 // body.status='ACCEPTED',
  ...
  const query = {
    name: 'getVotes',
    text: 'select max(vote_id) as vote_id, poll_id, secret_hash, salt, product_id, user_id, voter_publickey, record_status, status, voteoption, commitdate, revealdate from votes where product_id = $1 and poll_id = $2 group by poll_id, secret_hash, salt, product_id, user_id, voter_publickey, record_status, status, voteoption, commitdate, revealdate',
  ...
  const query = {
    name: 'getVotesUser',
    text: 'select max(vote_id) as vote_id, poll_id, secret_hash, salt, product_id, user_id, voter_publickey, record_status, status, voteoption, commitdate, revealdate from votes where product_id = $1 and poll_id = $2 and user_id = $3 group by poll_id, secret_hash, salt, product_id, user_id, voter_publickey, record_status, status, voteoption, commitdate, revealdate',
  ...
  const query = {
    name: 'commitVote',
    text: 'INSERT INTO VOTES (poll_id, secret_hash, salt, voteoption, product_id, user_id, voter_publickey, record_status, status) VALUES($1,$2,$3,$4, $5, $6, $7, $8, $9) RETURNING *',
  ...
  const query = {
    ...
    text: 'DELETE from VOTES where (poll_id = $1 and product_id = $2 and user_id = $3) and vote_id = (select max(vote_id) from votes where poll_id = $1 and user_id = $3 and product_id = $2) RETURNING *',
  ...
  const client = new Client({
   ...
   const query = {
    ...
    text: 'UPDATE VOTES SET record_status = \'confirmed\' WHERE (poll_id = $1 and voter_publickey = $2) and vote_id = (select max(vote_id) from votes where poll_id = $1 and voter_publickey = $2) RETURNING *',
