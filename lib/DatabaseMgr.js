...
class DatabaseMgr {
 ...
 const client = new Client({
  ...
  async projectChallengeConclusion(body) {
 ...
 // body.status='ACCEPTED',
  ...
  const query = {
    name: 'challengeEventConfirm',
    text: 'UPDATE  PROJECTS_DET set status = $2, rejection_date = $3, record_status = $4 WHERE project_id = $1 RETURNING *',
    values: [body.projectId, body.status, body.rejection_date, body.record_status]
  }
  ...
  const query = {
    ...
    values: [body.pollId, body.secretHash, body.salt, body.projectId,  body.userId,  body.voterPublicKey, 'pending_event']
  }
